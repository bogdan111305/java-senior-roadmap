...
#### Базовый цикл: init, clone, add, commit, push, pull, fetch
Ежедневная работа с репозиторием складывается из небольшого набора команд, у
каждой — своя роль в цепочке "локальные файлы → индекс → коммит → удалённый
репозиторий". `init` создаёт новый репозиторий с нуля, `clone` — скачивает уже
существующий вместе со всей его историей. `add` переносит изменения файла из
рабочей директории в индекс (staging area) — промежуточную зону, где
собирается будущий коммит, и именно поэтому можно закоммитить только часть
изменённых файлов. `commit` фиксирует текущее содержимое индекса как новый
снимок в истории. `push` отправляет локальные коммиты, которых ещё нет на
удалённом репозитории, `pull` — забирает чужие коммиты и сразу сливает их с
текущей веткой (это `fetch` + `merge` одной командой), а `fetch` — только
скачивает чужие коммиты в локальную копию удалённой ветки (`origin/main`), не
трогая текущую рабочую ветку, что даёт возможность сначала посмотреть, что
изменилось (`git log main..origin/main`), и только потом решить, сливать или
нет.

```bash
git init                     # новый репозиторий в текущей папке
git clone <url>               # скачать существующий репозиторий целиком

git add file.txt                # добавить конкретный файл в индекс
git add .                        # добавить все изменённые и новые файлы
git commit -m "fix null check in OrderService"

git push origin main              # отправить локальные коммиты на удалённый
git fetch origin                    # скачать чужие коммиты, не сливая их
git pull origin main                 # скачать и сразу слить (fetch + merge)
```
...
```bash
git checkout main
git merge --no-ff feature        # слить feature в main отдельным merge-коммитом

git checkout feature
git rebase main                    # переиграть коммиты feature поверх свежего main
# при конфликте на каком-то коммите: исправить файлы, затем
git add <файл>
git rebase --continue               # и так для каждого конфликтующего коммита
```
...
```bash
git stash                # отложить незакоммиченные изменения в стек
git stash list             # посмотреть, что лежит в стеке
git stash pop               # вернуть последний тайник и удалить его из стека
```

#### Интерактивный rebase: squash, fixup, reorder
`git rebase -i` открывает в редакторе список последних коммитов и позволяет
переписать их перед тем, как делиться веткой с другими — та же переигровка
коммитов, что и в обычном rebase, но с точечным контролем над каждым из них,
а не просто "поверх новой базы". `squash` объединяет коммит с предыдущим в
списке, оставляя оба сообщения коммита на редактирование в одно; `fixup` —
то же самое, но молча отбрасывает сообщение объединяемого коммита, оставляя
только сообщение предыдущего (удобно для правки в духе "забыл файл" без
отдельной пометки в истории); `reorder` — это не отдельная команда, а просто
изменение порядка строк в списке: коммиты применяются в том порядке, в
котором перечислены в редакторе.

```bash
git rebase -i HEAD~3
```
В открывшемся редакторе — список из трёх последних коммитов, каждый со своей
командой (по умолчанию `pick` — оставить как есть):
```
pick i7j8k9l refactor payment flow
squash e4f5g6h add integration test
fixup a1b2c3d fix null check in OrderService
```
После сохранения этого файла git применит `refactor payment flow`, затем
объединит в него `add integration test` (спросив итоговое сообщение для
объединённого коммита) и `fix null check in OrderService` (без вопроса,
молча взяв сообщение первого коммита) — вместо трёх отдельных коммитов в
истории останется один аккуратный. Типичный повод — причесать черновую серию
мелких "wip"-коммитов в один осмысленный коммит перед тем, как открыть pull
request.
