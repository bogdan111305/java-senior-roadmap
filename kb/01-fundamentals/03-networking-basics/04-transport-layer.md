...
# Транспортный уровень: UDP, TCP и ICMP
...
**Алгоритм Найгла (Nagle's algorithm).** Отправка каждого небольшого пакета
...
**Отложенное подтверждение (delayed ACK).** Симметричная оптимизация со стороны
...
**MSS и MTU.** MSS (Maximum Segment Size) — максимальный размер данных TCP,
...
**Keep-alive.** TCP-соединение может оставаться "открытым", но полностью
...
**Управление окном (window scaling).** Размер окна (receive window) — сколько
