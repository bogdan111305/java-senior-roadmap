...
```java
int[] arr = {1, 2, 3};
System.out.println("length=" + arr.length); // поле, а не arr.length()

int[] grown = Arrays.copyOf(arr, arr.length + 1);
grown[3] = 4;
System.out.println("arr=" + Arrays.toString(arr) + " (не изменился)");
...
grown=[1, 2, 3, 4]
```
`arr.length` — доступ к полю, без скобок. «Увеличить» массив нельзя — можно
только создать новый, большего размера, и скопировать в него старые элементы,
что и делает `Arrays.copyOf`; сам `arr` при этом остаётся прежней длины.
...
```java
int[] prims = {1, 2, 3};
int[] primsCopy = prims.clone();
primsCopy[0] = 99;
System.out.println("prims=" + Arrays.toString(prims) + " primsCopy=" + Arrays.toString(primsCopy));

StringBuilder[] objs = {new StringBuilder("a"), new StringBuilder("b")};
StringBuilder[] objsCopy = objs.clone();
objsCopy[0].append("!");   // мутируем объект, на который ссылается ячейка, а не саму ссылку
System.out.println("objs[0]=" + objs[0] + " objsCopy[0]=" + objsCopy[0]);
```
```
prims=[1, 2, 3] primsCopy=[99, 2, 3]
objs[0]=a! objsCopy[0]=a!
```
`primsCopy[0] = 99` меняет само значение в ячейке копии, никак не затрагивая
`prims` — ячейки независимы. А `objsCopy[0].append("!")` не трогает саму ссылку
в ячейке, а мутирует объект `StringBuilder`, на который указывают ОБЕ ячейки
(`objs[0]` и `objsCopy[0]`) — отсюда изменение видно в обоих массивах, хотя
сами массивы (как объекты-контейнеры) разные.
...
отсортирован по месту (in-place), без создания копии.
...
```java
int idx = Arrays.binarySearch(a, 8); // a уже отсортирован предыдущим примером
...
```
`binarySearch` возвращает индекс `8` в уже отсортированном `a` — искать в
исходном, неотсортированном массиве этим методом было бы некорректно (см.
ниже). `copyOf(a, 3)` берёт первые 3 элемента. `copyOfRange(a, 1, 4)` берёт
полуоткрытый диапазон индексов `[1, 4)` — элементы с индексами 1, 2, 3.
...
```
`System.arraycopy` скопировал 3 элемента из `src` начиная с индекса 1
(`2, 3, 4`) в начало `dst` — оставшиеся два элемента `dst` остались нулями по
умолчанию, потому что копирование их не затронуло.
...
(jagged array, см. пример выше); код, который предполагает единую ширину для
всех строк без явной проверки, может упасть с `ArrayIndexOutOfBoundsException`
на «рваных» данных, которые формально валидны для этого типа.
...
ячейках — те же самые ссылки на те же объекты, что и в оригинале.
...
```
Здесь `refsCopy[0] = "z"` не затронул `refs`, потому что это переприсваивание
ссылки в ячейке, а не мутация самого объекта `"a"` (строки к тому же
неизменяемы). Но, как показано в примере выше с `StringBuilder[]`, если
элементы — изменяемые объекты и код мутирует их напрямую через их методы, а не
переприсваивает ячейку, shallow copy уже не защищает: обе копии массива
...
System.out.println("3 есть в массиве, но binarySearch(unsorted, 3) = " + idx);
```
```
3 есть в массиве, но binarySearch(unsorted, 3) = -3
```
Значение `3` реально присутствует в массиве (индекс 3), но `binarySearch`
проверяет только середину диапазона на каждом шаге и, не найдя `3` там, где
предполагает упорядоченность, тихо докладывает «не найдено» (отрицательный
результат) — без единой проверки, действительно ли массив отсортирован.
