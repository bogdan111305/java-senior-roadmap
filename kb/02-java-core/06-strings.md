...
var sbAppend = StringBuilder.class.getMethod("append", String.class);
var sbufAppend = StringBuffer.class.getMethod("append", String.class);
System.out.println("StringBuilder.append synchronized: " +
        java.lang.reflect.Modifier.isSynchronized(sbAppend.getModifiers()));
System.out.println("StringBuffer.append synchronized: " +
        java.lang.reflect.Modifier.isSynchronized(sbufAppend.getModifiers()));
```
```
StringBuilder: 0,1,2,
StringBuffer: 0,1,2,
...
```
Итоговое содержимое идентично — оба класса дают одну и ту же изменяемую
последовательность символов и одинаковый API (`append`, `insert`, `delete`,
`toString()`). Разница видна через рефлексию, а не через результат: у
`StringBuffer` метод `append` реально объявлен `synchronized` и берёт монитор
объекта на каждый вызов, даже в однопоточном коде, как здесь, — лишняя работа,
которая ничего не покупает, если объект не расшарен между потоками.
...
где строки создаются как литералы и совпадают по ссылке благодаря пулу (см.
пример выше с `a == b`), а потом ломаться в проде на строках, полученных
другим путём (как `c`, созданный через `new`) — единственный надёжный способ
сравнения содержимого строк — `equals`.
...
by design synchronized (см. пример выше) и по-прежнему уместен там, где один и
тот же изменяемый буфер строки реально расшаривается между несколькими
потоками — просто такие случаи в современном коде редки по сравнению с
локальным использованием внутри одного метода, где синхронизация не нужна
вовсе.
...
replaceAll: -----
```
`replace(".", "-")` заменил ровно три буквальные точки. `replaceAll(".", "-")`
интерпретировал `.` как regex-метасимвол «любой символ» и заменил вообще все
пять символов строки — именно такая путаница чаще всего и ломает код,
работающий, например, с версиями (`"1.0.5"`) или путями к файлам.
