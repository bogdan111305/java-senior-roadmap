...
static void show(String x) { System.out.println("show(String): " + x); }
// ...
show(5);
show("hi");
```
...
show(String): hi
```
Это два разных метода с одним именем `show`, но разными сигнатурами (`int` и
`String`) — компилятор выбирает нужный вариант по типу аргумента в точке
вызова ещё до запуска программы, никакого возвращаемого типа в этом выборе не
участвует вовсе.
...
```java
static void show(String s) { System.out.println("show(String): " + s); }
static void show(Object o) { System.out.println("show(Object): " + o); }
// ...
Object obj = "hello"; // объявленный тип Object, фактический тип String
show(obj);
show((String) obj);
```
```
...
show(String): hello
```
Фактический объект в обоих вызовах — один и тот же `String "hello"`, но
`show(obj)` уходит в `show(Object)`, потому что overloading смотрит на
объявленный тип переменной (`Object`), а не на то, что реально лежит в куче —
только явное приведение `(String) obj` заставляет компилятор выбрать более
специфичную перегрузку.
...
```java
static void recurse(int depth) { recurse(depth + 1); }
// ...
try {
    recurse(0);
} catch (StackOverflowError e) {
    System.out.println("поймали StackOverflowError, но состояние потока уже ненадёжно");
}
System.out.println("программа продолжила работу дальше");
```
```
поймали StackOverflowError, но состояние потока уже ненадёжно
программа продолжила работу дальше
```
Код формально работает — `catch` перехватывает `StackOverflowError`, и
исполнение продолжается за пределами `try`. Но это не делает такой перехват
надёжной практикой: непосредственно после переполнения стек и часть состояния
JVM в неопределённом виде, и полагаться на корректность дальнейшей работы того
же потока не стоит — правильное решение — не допускать неограниченной
рекурсии, а не ловить её последствия.
